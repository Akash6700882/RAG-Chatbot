...
def get_recent_history(
    db: Session, owner_id: str, session_id: str
) -> list[dict[str, str]]:
...
def save_message(
    db: Session, owner_id: str, session_id: str, role: str, content: str
) -> ChatMessage:
    message = ChatMessage(
        owner_id=owner_id, session_id=session_id, role=role, content=content
    )
    db.add(message)
    db.commit()
    db.refresh(message)
